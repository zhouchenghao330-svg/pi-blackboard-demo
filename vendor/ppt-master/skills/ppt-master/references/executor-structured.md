...
**Default — re-skin `layout` (may override when the application plan keeps template visuals and the lock reflects them)**: inherit geometry, label/legend placement, and series encoding; repaint gradients, shadows, fills, and strokes from the current style/lock — Master/Layout atoms included, whose contract is their ids, order, geometry, and `data-pptx-*` metadata, never their paint. `mirror` preserves visuals under §1.1.
