...
| Self-running signage, animated, no narration | Custom Animations on, notes and audio off, `--auto-advance` dwell above each page's animation total plus reading time |
...
Default never forces object animation or audio because a deck may later be recorded; the Quick requirement selects the capability, not motion coverage.
