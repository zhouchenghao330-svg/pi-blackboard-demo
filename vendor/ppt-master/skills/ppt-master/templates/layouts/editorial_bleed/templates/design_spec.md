...
| Four overlay techniques | `hero_full` and `closing_full` use a bottom vertical fade (black `0 → 0.60 → 0.80 / 0.82`, the middle stop sitting where the title slot begins). `hero_side_scrim` uses a horizontal directional scrim (`0.88 → 0.62 → 0`, holding through the text slots' right edge) so the picture stays readable on the open side. Every fade keeps at least `0.60` behind its own text slot and falls away only past it. `chapter_full` uses a full-canvas graded wash (`0.44 → 0.68`) that darkens toward its text. `quote_over_image` uses a raised-floor radial vignette (`0.42 → 0.72`) so a centered quote holds contrast while the frame edges fall away. |
