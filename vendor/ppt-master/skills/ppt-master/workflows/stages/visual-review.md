...
**Host compatibility**: `TeamCreate` and `SendMessage` are Claude-Code primitives; a host with parallel anonymous subagents runs the batches in parallel with the main agent aggregating their returned reports; elsewhere the main agent processes the same batches sequentially with the same prompts — token savings persist, wall-clock grows roughly N/K-fold.
